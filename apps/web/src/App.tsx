...
const fmtPct = (value: number | null) => value === null ? "Awaiting model" : `${(value * 100).toFixed(1)}%`;
  ...
  return (
    <Shell>
      <header className="status-bar">
        <Status icon={<BadgeCheck size={17} />} label="Demo mode" value={state.demoMode ? "Enabled" : "Off"} />
        ...
        <Status icon={<Play size={17} />} label="Replay" value={`${state.replayStatus} @ ${state.speed}x`} />
        <Status icon={<Zap size={17} />} label="Theo" value={state.theoProvider} />
        <Status icon={<AlertTriangle size={17} />} label="Kill switch" value={state.killSwitch ? "Enabled" : "Off"} />
      </header>

      <section className="hero">
        <div>
          <p className="eyebrow">World Cup Quantitative Trading Demo</p>
          <h1>Replay-driven market making and paper trading</h1>
      ...
      <main className="grid">
        <Panel title="Market Board" wide>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Market</th><th>Selection</th><th>Mkt</th><th>Theo</th><th>Edge</th><th>Bid</th><th>Ask</th><th>Width</th><th>Leans</th><th>Size</th><th>Status</th></tr></thead>
              <tbody>{state.marketRows.map((row) => (
                <tr key={`${row.marketId}-${row.selectionId}`}>
                  <td>{row.market}</td><td>{row.selection}</td><td>{fmtPct(row.marketProbability)}</td><td><Badge label={fmtPct(row.theoProbability)} source={row.provenance.source} /></td>
                  <td>{fmtNum(row.edge)}</td><td>{fmtNum(row.bid)}</td><td>{fmtNum(row.ask)}</td><td>{fmtNum(row.width)}</td>
                  <td>{row.inventoryLean.toFixed(3)} / {row.directionalLean.toFixed(3)}</td><td>{fmtNum(row.bidSize)} / {fmtNum(row.askSize)}</td><td>{row.status}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
        ...
        <Panel title="Price Chart" wide>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={state.chart}>
              ...
              <Line type="monotone" dataKey="theoProbability" stroke="#0f766e" dot={false} />
        ...
        <Panel title="Positions">
          {state.positions.length === 0 ? <Empty text="No paper fills yet. Run the replay to create maker and directional inventory." /> : state.positions.map((position) => (
        ...
        <Panel title="Performance">
          ...
          <Metric label="Maker / taker" value={`${fmtMoney(state.performance.makerPnl)} / ${fmtMoney(state.performance.takerPnl)}`} />
        </Panel>
        ...
        <Panel title="Audit Trail" wide>
          <div className="audit-list">{state.audit.slice(-18).reverse().map((event) => (
            <article key={event.auditId}>
              ...
              <span>{event.marketObservation} | Theo {fmtNum(event.theo)} | Edge {fmtNum(event.edge)} | Width {fmtNum(event.width)}</span>
...
function Shell({ children }: { children: React.ReactNode }) {
  return <div className="app-shell">{children}<footer>Build {import.meta.env.VITE_APP_VERSION ?? "0.1.0"} | Paper-only deterministic replay</footer></div>;
}
