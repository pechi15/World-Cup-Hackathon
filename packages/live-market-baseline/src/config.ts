...
export function resolveMarketBaselineRuntimeConfig(env: Environment): MarketBaselineRuntimeConfig {
  ...
  if (dataMode !== "replay" && dataMode !== "txline") reasons.push(dataMode ? "INVALID_DATA_MODE" : "MISSING_DATA_MODE");
  exact(env, "DEMO_MODE", dataMode === "replay" ? "true" : "false", reasons);
  exact(env, "TRADING_MODE", "paper", reasons);
  if (dataMode === "txline") exact(env, "ENABLE_MARKET_MAKING", "true", reasons);
  if (env.ENABLE_DIRECTIONAL_TRADING?.trim().toLowerCase() === "true") reasons.push("INVALID_ENABLE_DIRECTIONAL_TRADING");
  if (env.ENABLE_KELLY?.trim().toLowerCase() === "true") reasons.push("INVALID_ENABLE_KELLY");
  ...
  return {
    ...
    maxLatencyMs: positiveNumber(env, "MARKET_DATA_MAX_LATENCY_MS", 2_000),
