...
export type QuoteCancellation = {
  ...
  marketId: string;
  cancelledAt: string;
  reasonCode: "QUOTE_EXPIRED" | "QUOTE_REPLACED" | "STALE_DATA" | "SEQUENCE_GAP" | "CONNECTION_LOSS" | "INFORMATION_SHOCK" | "RISK_SUSPENSION";
};
...
export class LiveMarketBaselineRuntime {
  ...
  private readonly lastProbabilities = new Map<string, number[]>();
  ...
  async onObservation(market: MarketDefinition, observation: NormalizedMarketObservation): Promise<MarketBaselineAudit> {
    ...
    const previous = this.lastProbabilities.get(market.marketId);
    const delta = previous ? Math.max(...observation.probabilities.map((probability, index) => Math.abs(probability - (previous[index] ?? probability)))) : 0;
    const previousVolatility = this.volatility.get(market.marketId) ?? 0.01;
    const innovationZ = delta / Math.max(previousVolatility, 0.0025);
    const nextVolatility = Math.sqrt(0.9 * previousVolatility ** 2 + 0.1 * delta ** 2);
    this.volatility.set(market.marketId, nextVolatility);
    this.lastProbabilities.set(market.marketId, [...observation.probabilities]);
    if (previous && innovationZ >= this.config.informationShockZ) {
    ...
    const theo = await this.provider.getTheo({ market, asOf: observation.receiveTime });
