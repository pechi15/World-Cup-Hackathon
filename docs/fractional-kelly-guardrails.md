...
- `THEO_MODE=market_baseline` always returns
  `DISABLED_NON_INDEPENDENT_THEO` and a null size with
  `INDEPENDENT_THEO_UNAVAILABLE`, `MARKET_BASELINE_IS_NOT_ALPHA`, and
  `KELLY_DISABLED`.
...
The dashboard intentionally shows:

```text
KELLY MODULE: AVAILABLE BUT LOCKED
Requires independently calibrated theo
```

This means the calculation utility exists, while runtime sizing remains
disabled (`kellyEnabled=false`, `kellySize=null`) and directional action remains
`NO_ACTION`. No environment variable can bypass the market-baseline lockout.
